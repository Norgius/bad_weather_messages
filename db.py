...
class Database:
    ...
    async def get_pending_sms_list(self):
        ...
        for key, phone2status in zip(keys, phones_pairs_groups):
            *_, sms_id_key = key.decode().split('_')

            pending_phones = (phone.decode() for phone, status in phone2status.items() if status == b'pending')
    ...
    async def list_sms_mailings(self):
        ...
        return [key.decode().split('_')[-1] for key in keys]
